export default class APIHandler {
  ...
  async postCard(cardObj) { 
    const request = new APIRequest("POST", "/kanban/cards", {title : cardObj.title, category : cardObj.category});
    const response = await APIProcessor(request);
  ...
  async putCard(cardObj) {    
    const request = new APIRequest("PUT", `/kanban/cards/${cardObj.id}`, {title : cardObj.title, category : cardObj.category});
  ...
  async deleteCard(id) {
    ...
    return console.log(this.dummyData)
  }
...
const HOST = 'https://zvg5cvf0yl.execute-api.ap-northeast-2.amazonaws.com/prod'
